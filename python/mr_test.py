...
class Read_Traffic_Check:
    ...
    def run(self):
        start = time.time()
        self.tc.mr_setup(self.udmabuf.phys_addr, self.array.size * (np.dtype(self.dtype).itemsize))
        self.tc.mr_monitor_clear()
        self.uio.irq_on()
        self.tc.mr_start()
        self.uio.wait_irq()
        self.tc.mr_clear_status()
        self.run_time = time.time() - start
        self.tc.mr_read_monitor()
                      
if __name__ == '__main__':
    print_tag = "Read_Traffic_Check :"
    parser = argparse.ArgumentParser(description='Test Bench for Traffic_Checker')
    parser.add_argument('-W', '--words'  , help='Read size(Words) ', action='store', type=int, default=1024)
    parser.add_argument('-B', '--bytes'  , help='Read size(Bytes) ', action='store', type=int, default=0)
    parser.add_argument('-K', '--kbytes' , help='Read size(KBytes) ', action='store', type=int, default=0)
    parser.add_argument('-M', '--mbytes' , help='Read size(MBytes) ', action='store', type=int, default=0)
    parser.add_argument('-N', '--number' , help='number of loops '   , action='store', type=int, default=1)

    args   = parser.parse_args()
    test   = Read_Traffic_Check()
    ...
    print ("{0} read_words             : {1}".format(print_tag, words))
    ...
    test.tc.mr_print_monitor(print_tag)
